...
- 네번째는 또한 line의 첫 문자가 따옴표 인지 검사하는 것으로
...
때 =(void *)0= 혹은 =NULL= 로, 문자일 때 ='\0'=
으로, 부동소수점 실수일 때 =0.0= 으로 써 의미가 명확히 드러나도록
하라.
...
- 프로그램을 작성하는 단계에서부터 버그를 줄이기 위하여 갖은 수단을
  동원해야 한다.
- 프로그램을 컴포넌트 단위로 쪼개고 컴포넌트끼리 덜 얽히도록 하여
  프로그램의 복잡성을 줄인다.
- 소프트웨어의 무결성을 보장하기 위한 프로그램 증명, 모델링, 요구사항
  분석, 정형 검증 등을 도입한다.
- 경계조건 테스트, 단정문, 정상성(sanity) 체크, 방어적 프로그래밍,
  인터페이스 설계, 검사 도구 등으로 버그를 미연에 찾아낸다.
- 버그는 거의 필연적이다. 결국 테스트로 그것을 찾고, 디버깅으로 잡아야
  한다.

** 디버거

- C, C++, Java와 같은 유명한 프로그래밍 언어들을 위한 강력한
  디버거들이 즐비하다.
  - 스택 추적, 변수 검사, 브레이크 포인트 등 다양한 기능 지원
- 디버거를 쓸 수 없는 경우도 있다.
  - 비주류 언어들은 디버거가 없거나 아주 원시적인 디버깅 기능을
    제공한다.
  - 디버거는 시스템 의존적이므로 어떤 시스템에선 쓸 수 없을지도
    모른다.
  - 멀티 쓰레드, 멀티 프로세스 프로그램은 디버거로 디버깅이 곤란한
    경우가 많다.
  - 운영체제, 분산형 시스템은 대개 더 저수준의 접근 방식으로
    디버깅해야 한다.
- 극한 상황에서는 개발자의 경험과 감, print 구문 외에는 믿을 것이
  없다.
- 프로그램 실행 흐름을 한 단계씩 깊이 들어가 보는 것보다, 열심히
  생각한 다음 중요한 위치에 자기검증 코드나 출력문을 추가하는 편이 더
  생산적이다.
- 디버거는 프로그램에 문제가 생겼을 때 그 상태를 확인하기 위한 용도로
  쓰는 편이 더 낫다. 나머지는 개발자의 역량으로 해결해야 한다.

** 실마리가 뚜렷한 쉬운 버그

*** 자주 나오는 패턴을 찾으라

흔한 버그에는 눈에 띄는 특징이 있다.

*예시 1*

 =printf= 와 =scanf= 함수에서의 타입 불일치
...
C언어의 흔한 버그 패턴으로, 지역 변수 초기화를 빠뜨려도 에러가 발생할
수 있다.

*** 가장 최근에 변경한 부분을 검사하라

프로그램을 개발하면서 하나에 하나씩만 고쳤다면 버그는 새 코드에
있거나, 새 코드에서 드러났을 가능성이 크다. 최근 변경을 주의깊게
살펴보라. 그러므로 정상 동작한다고 믿을 수 있는 과거 버전을
기록해두고, 어떤 점이 바뀌었는지 비교 평가해야한다.

*** 같은 실수를 두 번 반복하지 말라

한 버그를 고치면 유사한 실수를 다른 곳에서도 반복한 것은 아닌지
자문해보라. 똑같은 버그를 또 겪게 될지도 모른다.

*** 오늘 할 디버깅을 내일로 미루지 말라

프로그램이 죽었을 때 무시하고 넘어가지 말라. 손쓰기에 너무 늦어지기
전에 다시 발생하는 일이 없도록 바로 프로그램을 뒤지면서 문제를
추적하라.

*** 스택 추적값을 확인하라

디버거의 가장 일반적인 용도 중 하나는 프로그램이 죽은 다음의 상태를
확인하는 것이다. 문제가 생긴 소스 줄 번호는 가장 유용한 디버깅
정보이며, 잘 나옴직하지 않은 인자값도 좋은 실마리가 된다.

*** 작성하기 전에 읽으라

당장 고쳐서 돌려보고 문제가 없으면 곧장 넘어가고 싶은 생각이
간절하겠지만, 코드를 주의 깊게 읽고 원인을 생각해 보는 것이 더 도움이
된다. 코드의 일부분에 집중하지 말고 전체적 흐름을 살펴보라.

*** 코드를 다른 사람에게 설명하라

자신의 코드를 볼 때 섣부른 가정으로 작성한 코드 부분이 있다면 코드를
남에게 설명하면서 어느 부분이 덜 엄밀했는지를 파악할 수 있다. 그
부분을 규명함으로써 때로 버그가 발견되기도 한다.

** 실마리가 없는 어려운 버그

*** 버그를 재현할 수 있게 하라

약간의 시간을 할애하여 문제를 확실히 드러내는 입력과 매개 변수 값을
마련하여 버튼 하나만 눌러도 버그를 발생시킬 수 있도록 하라. 정말
심각한 버그라면 문제를 어차피 계속 재현해봐야 하기 때문에 돌아가는 것
같이 보이지만 오히려 미리 준비하여 시간을 절약하는 길이기도 하다.

버그를 매번 발생시킬 수 없다면, 왜 안되는지 생각해보아야 한다. 어떤
조건이 갖춰져야 더 자주 발생시킬 수 있을지 알아내고, 버그가 재현될
때까지 멍하니 기다리는 시간을 줄여야 한다.

디버깅용 출력을 지원한다면 사용하라. 난수를 이용하는 프로그램이라면
seed를 일정하게 제공하도록 하여 결과를 재현할 수 있도록 하라.

*** 각개 격파하라

문제가 되는 입력의 범위를 좀 더 좁혀보라. 에러에만 집중한 주요 테스트
케이스를 찾아보고, 무엇이 잘못된 것인지에 대한 논리적 가설을 계속해서
테스트해보라.

가능하다면 이진 탐색 방식을 사용해 입력 범위의 반을 날리고, 또 반을
날리고 하면서 문제가 되는 최소 입력을 찾아내라.

*** 수(數)가 의미하는 바를 연구하라

문제가 되는 입력 숫자가 일정한 패턴을 가지고 있다면, 유사한 수를
코드에서 찾는다. 어쩌면 그 숫자가 디버깅을 위한 열쇠가 될 수도 있다.

*** 결과를 출력해서 탐색 범위를 좁혀라

프로그램이 무슨 일을 하는지 이해할 수 업삳면, 더 많은 정보를 볼 수
있도록 출력문을 추가하는게 가장 쉽고 효율적인 디버깅 방법이다.
출력문이 이상한 결과를 출력한다면, 그 값이 나온 길을 역추적하고
생각대로 안 된 이유를 검증한다.

출력문을 추가하는 것은 변수 값을 추적하거나, 코드의 어느 위치까지
실행됬는지 확인하는 가장 쉬운 방법이다.

*** 자가검증 코드를 작성하라.

필요하다면 내가 만든 프로그램의 일부분에서 가정한 기본 전제가
어긋났는지 확인하는 코드를 써라. 다음과 같이 말이다.

#+BEGIN_SRC c
  /* check: 조건을 테스트하고 출력한 다음 실행을 중단한다 */
  void check(char *s)
  {
      if (var1 > var2) {
          printf("%s :var1 %d var2 %d\n", s, var1, var2);
          fflush(stdout);         /* 끝까지 출력이 나오도록 함 */
          abort();                /* 비정상 종료를 알림 */
      }
  }
#+END_SRC

abort는 디버거로 분석 작업을 할 수 있도록 프로그램을 비정상
종료시킨다. 이 검증 코드는 =var1= 이 =var2= 보다 작거나 같음을 전제로
하고 있다는 것을 알 수 있다. 작은 검사 코드를 이곳 저곳에
심어놓음으로서 언제 전제가 파괴되는지 확인할 수 있다.
...
더 어려운 문제를 조사하고자 한다면, 자료구조까지 출력하도록 만들 수
있다. 이를 일반화하면 데이터 구조나 다른 정보들 사이의 일관성을 계속
검증하는 루틴이 된다. 소프트웨어가 복잡한 데이터 구조를 포함하고
있다면, 이런 check 함수를 production 레벨에 가기 전까지 프로그램에
넣어두고 말썽이 생기기 시작할 때 값 하나를 바꾸는 것으로 바로 디버깅
모드로 돌입할 수 있도록 하는 것도 좋은 방법이다.

전화 교환기 시스템같은 큰 프로그램은 대개 여러 정보와 장치를
모니터링하면서 문제를 보고하고, 심지어 문제가 발생했을 때 자기수복까지
하는 감사(勘査)용 하위 프로그램에 상당 부분을 할애한다.

*** 로그 파일을 작성하라

또 다른 전략은 디버깅을 위해 고정 형식 스트림 출력을 담는 로그 파일을
작성하는 것이다. 웹서버나 기타 네트워크 프로그램은 트래픽 로그를
기록하여 자신과 클라이언트를 모니터링한다. 로그 파일이 있다면
프로그램에 죽기 직전에 어떤 일이 일어난것인지 사후조사할 수 있다.

물론 프로그램이 죽기 직전에 버퍼를 비워서 로그가 제대로 남도록 하는
것이 중요하다. C언어에는 fflush, C++이나 Java에는 flush 함수가 있다.
그게 아니라면 아예 처음부터 버퍼링이 없는 파일 스트림인 표준 에러를
이용하는 방법도 있다. 경우에 따라 시스템에 부하를 주기도하겠지만
말이다.

*** 그림을 그려라

통계를 위해 산점도를 그리면 숫자를 나열한것보다 더 직관적으로 이상값이
표현된다. 데이터 히스토그램은 시험 결과, 난수, 메모리 할당 루틴과 해시
테이블, 내부 버킷 크기 등의 이상 징후를 드러낸다.

*** 기록하라

버그를 찾으면서 시간이 흐르면 무엇을 시도했고 결과가 어땠는지를
잊어버리기 시작한다. 테스트와 결과를 기록해 둔다면 뭔가를 간과하거나
실제로는 확인하지 않은 것을 확인했다고 착각할 가능성이 줄어들 것이다.

** 최후의 수단

이도저도 안된다면, 한줄 한줄 면밀히 조사해가며 버그의 발생 위치를
찾아내는 수 밖에 없다. 문법이나 함수 사용법의 몰이해로부터 비롯된
버그는 찾기가 굉장히 어렵고, 인자의 순서가 바뀌었을 때 타입 검사가
도움을 주지 못하는 경우도 존재하기 때문에 주의해야 한다. 다른 곳에서
공유 변수가 몰래 바뀌기도 하며, 알고리즘이나 데이터 구조에 치명적
결함이 발생하기도 한다. 왠만해서 데이터구조나 알고리즘은 한번 써두면
절대로 다시 읽고 싶은 생각이 안 들기 때문에 에러가 없다고 검증할 수
있을 때까지 충분한 테스트를 거쳐야 한다.


** 재현 불가능한 버그

가만히 멈춰있지 않는 벌레를 잡기가 가장 어렵다. 도저히 증상을 정의할
수가 없다면, 그 에러가 알고리즘에 있는 것이 아니라 프로그램을 실행할
때마다 바뀌는 어떤 정보를 이용하는 부분에 있을 가능성이 높다는
의미이기 때문이다.

변수 초기화가 의외의 단서가 될 수 있다. 이전에 같은 메모리 위치에
저장돼 있던 값을 제비 뽑듯이 우연히 뽑은 걸지도 모른다. 모든 변수에
정의된 값을 지정하라. 난수발생기가 있다면, seed값으로 특정 값을 지정해
결정적(deterministic)이게 만들어야 한다.

그 밖에:

- 메모리 할당과 해제 루틴을 댕글링 포인터를 포함하여 면밀히 검증하라.
- 라이브러리가 외부 환경에 영향을 받는지 확인하라.
- C/C++의 경우, 스코프를 벗어나서 주고받는 데이터가 정말로 유효한지도
  살펴보라.
...
저장공간 관리 문제를 디버깅할 때 쓸 수 있는 malloc과 free 함수를
작성하라. 한 가지 접근 방법은 malloc과 free를 호출할 때마다 전체 작업
공간을 검사하는 것이다. 그리고 또 다른 방법은 다른 프로그램에서 쓸 수
있게 로그 정보를 기록하는 것이다. 어떤 방법을 택하든, 할당 공간의
시작과 끝에 표시를 해서 양쪽으로 넘치는 상황을 포착할 수 있어야 한다.

*인터페이스*

=dmemory= 라는 이름으로 라이브러리를 제작한다. 우리가 앞으로
사용자에게 제공할 인터페이스이다. =malloc= 함수를 대체하는 =dmalloc=,
=free= 함수를 대체하는 =dfree= 를 제공한다. 이 함수들은 off-by-one
에러를 추적하거나, dangling-pointer 에러를 추적하는 데 용이할 것이다.
추가 디버깅을 위해 메모리 침범이 있었는지 확인하는 함수와 전체 할당
메모리를 출력하는 함수도 제공한다.
...
이런 기능들은 사용자가 명시적으로 =__DEBUG_MODE= 매크로 상수를
선언해야만 동작하도록 만든다. =dmalloc= 과 =dfree= 함수가 기능을
제공하면서 추가적인 시스템 부하가 발생하기 때문이다.

*자료 구조*

=malloc= 래퍼 함수로 할당된 메모리 공간들을 전수 조사 하기 위해서는
메모리 블록들을 관리하는 자료 구조가 필요하다. 해시 테이블로 임의
접근이 가능하게 만드는 것도 좋지만, 구현 상의 편의를 위해 리스트로
할당된 블럭을 관리하기로 한다. =memlist.c= 파일에 자료구조
라이브러리를 작성하자. 링크드 리스트를 선언한다.
...
리스트 뒤에 쉽게 추가하기 위하여, 리스트의 첫 노드와 끝 노드에 빠르게
접근할 수 있는 지름길을 만들어둔다.
...
자료구조에 버그가 생기면 고치기가 매우 어렵기 때문에, 개발을 끝낼 때
까지 단정문들을 남겨둘 것이다. 빈 리스트라면 =memlist= 와 =memtail=
모두 =NULL= 이어야 하고, 리스트에 원소가 하나라도 있다면 =memlist= 와
=memtail= 모두 =NULL= 이 아니어야 한다는 전제로 개발한다.

리스트에서 메모리 블록 노드를 찾는 =find= 함수를 만든다.
...
이 함수의 결과를 가지고 리스트 원소를 지울 수 있도록 =del= 함수를 작성한다.
...
4장에서 다룬 인터페이스 설계 원칙 "자원을 결자해지하라"는 말을 따라서,
실제 객체는 더 상위의 함수들이 해제하도록 할 것이다. =makemem= 으로
객체를 생성할 때도 이미 리스트 라이브러리 외부에서 자원이 할당되어
들어옴을 가정했기 때문이다.

메모리를 전수조사하거나 모두 출력할 때 도움이 될 수 있도록 메모리
블록을 모두 순회하는 함수를 만든다.
...
자료구조 라이브러리에 의한 버그를 미연에 방지하기 위해, 테스트
함수들을 만들 것이다. 라이브러리는 원래 main 함수를 가져선 안되기
때문에 컴파일 과정에서 이를 제어할 수 있도록 매크로 조건을 추가한다.
...
컴파일 할 때 메모리 블록 리스트에 대한 테스트를 수행하려면 다음과 같이
옵션을 주면 된다.

#+BEGIN_EXAMPLE
  CMD> gcc -D__MEMLIST_TEST_MAIN dmemory.c -o dmemory.exe
#+END_EXAMPLE

우선 테스트를 용이하게 하기 위해 =Dmem= 객체 출력 함수를 만든다.
...
정상적인 출력이 나타난 모습이다. 이제 이 함수를 이용해 테스트를 수행할
수 있다.

*자료구조 테스트 작성*

메모리 블록 리스트를 테스트 하기 위한 함수를 작성한다.
...
어떤 assertion에서도 걸리지 않았고, 제대로 된 출력이 나타난 모습이다.

*every 함수 테스트*

아직 =every= 함수를 테스트하지 않았다. 테스트를 작성하고, 수행한다.
...
=dmemory= 라이브러리 구현에 도움을 줄 함수들이다. =dmemory.c= 에
작성한다.

메모리를 검증하는 =isclean= 함수를 작성한다. 이 버퍼 오버플로우를
감시할 때 유용할 것이다. 검증 바이트는 임의로 결정했다. 처음에는
=0xff= 를 사용했지만, 이는 비트를 모두 채우기 위해 흔히 사용하는
값이라는 조언을 들었다. 그래서 그보다 1 작은 =0xfe= 로 고쳤다.
...
=Dmem= 객체의 정보에 따라 메모리 상태를 출력해주는 함수를 만든다.
보이는 형식에 조금 신경을 써서 만들었다.
...
이 함수 중간에 쓰인 =spaces= 함수는 지정된 개수만큼 스페이스를 찍은
문자열을 돌려주는 함수이다.
...
천천히 하자. 디버그에 쏟는 시간을 줄이기 위해 테스트를 작성한다.
실제로, 나는 테스트의 덕분에 =isclean= 함수의 버그를 미리 잡을 수
있었다.
...
#+BEGIN_SRC c
  ...
  void test_printmem()
  {
      ...
      bytes[ssize] = '\0';        /* off-by-one 버그를 발생시킨다. */
      printmem(newm, stdout);
  }
...
#+BEGIN_EXAMPLE
  gcc -D__DMEMORY_TEST_MAIN dmemory.c memlist.c -o dmemory.exe
#+END_EXAMPLE
...
#+BEGIN_SRC c
  0x11e12a1: is clean.
             68 65 6c 6c 6f 2c 20 77   h e l l o ,   w 
             6f 72 6c 64 00 00 00 00   o r l d . . . . 
             00 00 00 00 00 00         . . . . . . 
  0x11e12a1: is not clean.
             78 78 78 78 78 78 78 78   x x x x x x x x 
             78 78 78 78 78 78 78 78   x x x x x x x x 
             78 78 78 78 78 78         x x x x x x 
#+END_SRC

출력 결과에는 검증 바이트가 드러나지 않지만, 검증 바이트가 =0xfe= 에서
=0x00= 으로 파괴되었기 때문에 =isclean= 함수가 버퍼 오버플로를 감지할
수 있다.

*라이브러리 구현*

우선 무결성 파괴를 감지하여 무결성이 깨지면 메모리 블록을 출력하고
종료하는 줄임말 함수부터 만들자.
...
이 함수는 =dmalloc= 과 =dfree= 함수에서 쓰일 것이다.

=dmalloc= 함수를 구현한다. 무결성 체크를 수행하고, 메모리에 추가 검증
바이트를 삽입하고, 메모리 블록 리스트에 넣어서 지속적으로 관리되도록
한다.
...
=dfree= 함수를 구현한다. 이 역시 무결성 체크를 수행하고, 메모리 블록
리스트에 없다면 댕글링 포인터 에러를 발생시키고, 메모리에 추가 검증
바이트를 제거하고, 메모리 블록 리스트에서 제거한다.
...
=dmem_isclean= 함수와 =dmem_dump= 함수는 미리 만들어둔 헬퍼 함수들을
이용하면 간단하다.
...
이 함수들은 사용자가 직접 상황을 제어하기 위해서 호출하는
함수들이므로, 무결성 검사 및 강제종료를 하지 않는다.

*Dmemory 라이브러리 테스트*

가장 먼저 =dmalloc= 의 수행 과정부터 검정한다.
...
#+BEGIN_EXAMPLE
  0xa9d0f231: is not clean.
              68 65 6c 6c 6f 2c 20 77   h e l l o ,   w
              6f 72 6c 64               o r l d
  dmalloc: Detected broken memory integrity
#+END_EXAMPLE

정상적으로 무결성 검사가 일어난다.

다음에는 댕글링 포인터를 제대로 검사하는지 검정한다.
...
#+BEGIN_EXAMPLE
  Here we go...
  dfree: dangling pointer error
#+END_EXAMPLE

정상적으로 댕글링 포인터 검사가 일어난다.

*시험 주행*

이제 정상적으로 기능하는 시나리오를 만들어본다. 파일에서 단어별 등장
횟수를 세는 프로그램을 만들면서 =dmalloc= 과 =dfree= 를 이용해본다.
...
"Alice in Wonderland" 내용을 삽입해본다.

#+BEGIN_EXAMPLE
  CMD> wordcount.exe alice.txt 10
  [001] the : 1505 times.
  [002] and : 712 times.
  [003] to : 703 times.
  [004] a : 603 times.
...
=dmalloc= / =dfree= 함수가 아주 많이 호출 됬을 텐데도 정상적으로
실행된다. 그러나 프로그램이 모든 자원을 잘 해제 했는지, =dmem_dump= 가
아무것도 출력하지 않는다.

=rank= 함수가 =words= 를 해제하지 않도록 일부러 누수를 만든 뒤
실행하면 =dmem_dump= 함수의 출력이 나타난다.
...
#+BEGIN_EXAMPLE
  ...
  [009] in : 343 times.
  [010] was : 328 times.
  000001FCA01FD021: is clean.
                    f1 c7 11 a0 fc 01 00 00   . . . . . . . .
                    51 c8 11 a0 fc 01 00 00   Q . . . . . . .
                    21 bd 11 a0 fc 01 00 00   ! . . . . . . .
                    01 e0 11 a0 fc 01 00 00   . . . . . . . .
                    01 c7 11 a0 fc 01 00 00   . . . . . . . .
  (중간 생략)
                    01 25 17 a0 fc 01 00 00   . % . . . . . .
                    01 62 1f a0 fc 01 00 00   . b . . . . . .
                    81 bb 13 a0 fc 01 00 00   . . . . . . . .
                    e1 00 1f a0 fc 01 00 00   . . . . . . . .
#+END_EXAMPLE

8바이트씩 끊어진 주소값들이 보인다(바이트가 역순으로 보이는 까닭은
엔디안 형식 때문이다).

"Alice in Wonderland"는 아주 커다란 파일이기 때문에, 단어가 매우 많아
출력 내용이 아주 길다. 실제로 써먹기 위해서는, =dmem_dump= 함수가 세부
메모리 내용을 출력할지 말지 결정하는 옵션을 도입하는 것이 좋겠다.

** 디버깅 툴

유닉스 시스템에는 대량의 데이터를 선택하고, 이상을 찾고, 재배열 할 수
있게 도와주는 수 많은 프로그램들이 있다. 디버거가 아니더라도 이런
도구들을 이용해 프로그램을 분석할 수 있다.

이 절에서는 이진 형식을 다룰 때 특히 유용한 =stirngs= 라는 프로그램을
소개한다. 이 절에서 소개하는 =strings= 는 유닉스 시스템이 기본적으로
갖춘 =strings= 와는 사실 조금 다르다는 것을 유념하라. 유닉스의 내장 툴
=strings= 는 프로그램의 심벌 테이블은 무시하고 텍스트 세그먼트(기계어
코드 영역)와 데이터 세그먼트(정적 데이터 영역)만을 검사한다. -a
옵션으로 파일 전체를 읽게 할 수 있기는 하다.

*** strings 프로그램

=strings= 는 이진 파일에서 출력가능한 ascii 텍스트를 추출해 다른
프로그램이 읽거나 처리할 수 있게 한다. 에러메시지가 어떤 프로그램에서
나타났는지 살펴보고 싶을 때는 다음과 같은 명령 구문으로 출처를 찾아낼
수 있다.
...
이것을 직접 만들어본다. 아래에 소개하는 *strings* 함수는 파일을 읽고
=MINLEN= = 6 이상의 출력 가능한 문자열을 모두 출력한다.

#+BEGIN_SRC c
  /* strings: 스트림에서 출력 가능한 문자열을 추출한다. */
  void strings(char *name, FILE *fin)
  {
      ...
      do { /* 한 번에 한 문자열씩 처리 */
          ...
          if (i >= MINLEN)        /* 길이가 충분하면 출력한다 */
              printf("%s:%.*s\n", name, i, buf);
      } while (c != EOF);
  }
#+END_SRC

=".*s"= 포맷 지정자는 i길이의 문자열 buf 를 출력한다.

#+BEGIN_QUOTE
위쪽에서 검사하는 표준적인 루프를 썼거나 getc가 들어간 더 복잡한
루프를 한 개만 썼다면 똑같은 printf를 여러 번 써야했을 것이다.  사실
처음 이 함수를 작성할 때는 그런 식이었는데 ... ...  중복 코드를 줄이는
방향으로 프로그램을 완전히 새로 작성할 필요가 있다는 게 분명해졌고,
그래서 이런 do-while 루프를 넣었다.
#+END_QUOTE

다음 main 함수는 인자로 넣은 각 파일에 대해 strings 함수를 호출한다.

#+BEGIN_SRC c
  /* strings main: 여러 개의 파일에서 출력 가능한 문자열을 찾는다. */
  int main(int argc, char *argv[])
  {
      int i;
      FILE *fin;
...
=strings= 프로그램은 =MINLEN= 이상의 길이로 된 문자열을 출력하기
때문에 가끔은 필요한 것 보다 더 많은 결과가 나온다. 옵션 인자를 주어
최소 문자열 길이를 정의할 수 있게 고쳐 보라.

*해결책*

옵션을 처리할 때 ~--minlen=XX~ 혹은 ~-mXX~ 옵션으로 =MINLEN= 을 받도록
한다. 아래 구현은 편의상 ~-mXX~ 옵션만 처리하도록 구현했다.

#+BEGIN_SRC c
  void usage()
  {
      eprintf("usage: strings [-mXX filename]*");
  }
      ...
      if (argc == 1)
          ...
          for (i = 1; i < argc; i++) {
              if (strncmp("-m", argv[i], 2) == 0) {
                  strptr = &argv[i][2];
                  minlen = strtol(strptr, &endptr, 10);
                  if (strptr != endptr)
                      MINLEN = minlen;
                  else            /* 숫자 파싱 실패 */
                      usage();
...
입력값을 복사해서 출력 값으로 내보내는 vis 프로그램을 작성하라. 다만
복사할 때 백스페이스, 제어문자, \Xhh (hh는 비출력용 바이트의 16진수
표현임)처럼 아스키 문자가 아닌 문자값을 포함한 비출력용 바이트들도
출력할 수 있어야 한다. strings와 달리 vis는 출력하지 않을 문자가 적은
입력을 검사할 때 제일 유용하게 쓸 수 있다.
...
#+BEGIN_SRC c
  ...
  /* to_hex: 1바이트 정수 c의 16진수 표현(\Xhh)을 des에 담는다. */
  char *to_hex(char* des, unsigned char c)
  {
      sprintf(des, "\\X%0x", c);
      return des;
  }

  /* vischar: 1바이트 문자 c를 출력한다. 비출력 문자는 16진수 표현으로
   ,* 바꾸어 출력한다. */
  void vischar(unsigned char c, FILE *fout)
  {
      ...
      if (isprint(c) || isspace(c))
          ...
          fputs(to_hex(buf, c), fout);
  }

  /* vis main: 입력으로 들어온 문자열을 모두 출력하되, 비출력 문자는
   ,* 16진수 표현으로 바꾸어 출력한다. */
  int main(void)
  {
      char c;
...
입력이 \X0A일때 vis 프로그램은 어떤 값을 출력하겠는가? vis가 모호하지
않은 출력값을 내놓게 하려면 어떻게 해야겠는가?

*해설*

문제 취지와는 다른 대답이지만, \X0A 문자열 자체는 그대로 출력되고,
만약 아스키 코드를 의미하는 거라면 내 프로그램에서는 줄바꿈 문자로
출력된다(LF 코드값에 해당).

비출력 문자와 입력 제공자가 임의로 넣은 16진수 표현을 구분하고자
한다면, 어떤 특수한 약속으로는 불가능하다. 왜냐하면 어떤 특별한 출력
값을 만들어내도 고의적으로 그 출력을 똑같이 따라 넣으면 되기
때문이다. 이를 위한 대책에는 두 가지 방법이 있다.

첫째는 텍스트 입력으로 도저히 불가능한 출력값을 내놓는
방법이다. 예컨대, 색상을 입힌 텍스트와 같은 출력이다. 그러면
프로그램을 실행시켰을 때 어떤 게 비출력 문자인지 정확히 파악할 수
있다.

둘째는 프로그램 옵션을 제어해서 비출력 문자에 대한 추가 검증을
가능하게 하는 것이다. 예를 들어 비출력 문자의 위치(행과 열)를 별도의
파일로 출력하게 한다거나, 16진수 혹은 8진수 표현을 선택할 수 있도록
하여 교차 검증이 가능케 하는 것이다. 이런 방법으로 vis가 모호한 출력을
내놓았을 때 프로그램 실행자가 이게 정말 비출력 문자가 맞는지 확인해 볼
수 있을 것이다.

*** 연습 5-5.

vis 프로그램을 확장해서 파일 여러 개를 순차적으로 처리하고, 긴 입력을
줄바꿈해서 처리하고, 비출력용 문자들을 제거하는 기능을 추가하라. 이
프로그램의 역할에 어울리는 다른 기능에는 무엇이 있겠는가?

*프로그램*

목적에 맞게 프로그램을 바꾸고 나니, 코드의 원래 형태와는 많이 달라졌다.
...
마지막 입력은 기다란 한 줄짜리 입력이다.

*수행 결과*

#+BEGIN_EXAMPLE
  CMD> vis.exe -f30 input1.txt input2.txt input3.txt
  hello, world
  all is well
  online
  SAMURAI Project
    Alice was beginning to get v
  ery tired of sitting by her si
  ster on the bank, and of havin
  g nothing to do: once or twice
   she had peeped into the book
  her sister was reading, but it
   had no pictures or conversati
  ons in it, `and what is the us
  e of a book,' thought Alice `w
  ithout pictures or conversatio
  n?'
#+END_EXAMPLE

*해설*

이 프로그램은 유닉스 툴킷 중 하나인 fmt와 닮아 있다. fmt는 한 줄의
길이가 너무 길지 않도록 단어 단위로 잘라서 보기 좋게 꾸며주는
프로그램이다. 이 프로그램은 단어 단위로 기능이 없고, 대신 비출력
문자를 생략해주는 기능이 들어가있다. 조금 더 코드를 가미하여 단어
단위로 자르는 옵션도 만들면, 비출력 문자를 허용하는 fmt와 비슷한
역할을 수행할 수 있을 것으로 보인다.

** 다른 사람의 버그

*** 소개

처음부터 모든 것을 새로 만드는 개발자는 거의 없다. 다른 사람이 만든
코드를 사용하고, 유지보수하고, 수정하고, 그리고 선택의 여지없이
디버깅하는 데 많은 시간을 쏟는다.

다른 사람이 만든 프로그램도 이 장에서 소개한 바와 비슷한 방식으로
디버그하면 되지만, 그 전에 먼저 소스코드의 의도를 파악하는 것이
우선이다.

*** 툴에 대하여

특정 이름이 등장하는 위치를 찾기 위해 grep이나 교차 참조 검색기가 매우
유용하다. 특히 교차 참조 검색기는 함수 호출 구조를 그래프로 그려주어
매우 유용하다. 다른 사람의 코드를 분석할 때는 디버거도 유용하게
쓰인다. 실행을 한 단계씩 좇으며 일이 어떤 순서로 일어나는지 밝힐 수 있다.

*** 버그 리포트

때로는 자신의 책임도 아니고 소스코드조차 없는 소프트웨어의 에러를
찾아야 할 때가 있다. 이런 경우의 목표는 정확한 버그 리포트를 쓸 수
있을 정도로 버그를 잘 식별하고 특정짓는 동시에, 그 문제를 피할 수 있는
'우회로'를 찾는 것이다.

제일 중요한 것은 그게 진짜 버그라는 사실을 확실히 검증하는 것이다.
제작자의 시간을 낭비하거나 양치기 소년이 되어 신뢰를 잃는 일이 없도록
말이다.

특히 컴파일러의 버그를 발견할 경우, 그것이 혹시나 버그가 아닌 호환성의
문제가 아닐지, Undefined Behavior은 아닌지 확인하기 위해 언어의 정의를
확인하라.

기존에 알려지지 않은 버그가 확실한지 확인하라. 프로그램이 최신 버전이
아니라면, 최신 버전에서는 고쳐진 버그일 수도 있고, Known Issue일수도
있다.

마지막으로 버그 리포트를 읽는 사람 입장이 되어 생각해본다. 제작자에게
가능한 한 재현이 쉽고 최소한의 입력으로 버그를 발생시키는 테스트
케이스를 제공해야 한다. 또한 관련이 있을만한 다른 정보를 같이 첨부하는
것이 좋다. 프로그램 버전, 컴파일러의 버전, 운영체제, 하드웨어 정보
등이다. 패치를 함께 제공한다면 더욱 좋다.
...
- 디버깅은 프로그램이 망가졌다는 것을 알았을 때 하는 일이다.
- 테스트는 제대로 돌아간다고 생각되는 프로그램을 의도적이고 체계적으로
  망가뜨려 보는 시도다.
- 다익스트라에 따르면, 테스트는 버그가 존재함을 보여줄 수는 있으나
  부재함을 보여줄 수는 없다.
- 처음부터 완벽한 검증을 통해 테스트가 필요없는 프로그램을 만드는 것이
  이상적이지만, 현실적인 목표는 아니다.

*** 버그가 없는 프로그램을 만들기 위하여

- 코딩하면서부터 잠재적인 문제를 파악하는 것이 버그를 없애는 좋은
  시작이 된다.
- 작은 테스트부터 복잡한 테스트까지 체계적으로 테스트하면 프로그램이
  확장되더라도 여전히 올바로 동작함을 보장할 수 있다.
- 자동화된 테스트는 손으로 일일이 값을 바꿔가며 해보는 테스트보다
  효율적이고 더 커다란 테스트를 시도하는 데 도움을 준다.
- DSL을 통해 코드 생성을 자동화한다면 더욱 좋다. DSL 해석기가 올바른
  한, DSL로 작성된 코드는 버그의 여지가 더욱 적을 것이다. 이는 9장에서
  다룬다.

** 코딩하면서 테스트하기

문제를 일찍 발견할 수록 좋다. 프로그램을 만들면서 몇몇 단순한 속성들을
검증하라.

*** 경계 조건에서 테스트하라

경계 조건 테스트(boundary condition test)라는 기법이 있다. 코드를
조금씩 쓸 때마다, 조건 분기가 제대로 되는지, 루프가 적절한 횟수만큼
도는지 검사한다. 빈 입력, 한개의 입력, 꽉 찬 배열 등 데이터와
프로그램에 내재된 태생적 경계를 검사할 수 있다. 경계가 아닌 곳에서
어떤 일이 일어나는지는 상상하기 쉽다. 그러나 경계에서 어떤 일이 일어날
지 상상하는 것은 비교적 어렵다.

*잘못된 예시*

아래 코드는 =fgets= 를 모델로 버퍼가 꽉 차거나 개행 문자가 나올 때까지
읽는다.
...
일반적인 경우 잘 돌아가리란 것은 쉽게 상상할 수 있다.

그러나 빈 문자열이 입력된 경우라면, 마지막 줄에서 인덱스가 -1이 된다.

이를 해결하기 위해 for-idiom을 이용해 코드를 다시 고쳐 써보자.
...
빈 문자열, 아주 긴 입력 등에서도 문제가 없다는 걸 알 수 있다.

또 다른 경계, EOF가 입력된 경우를 생각해본다. 그렇다면 이 루프는 절대로
끝나지 않을 것이다.
...
도중에 개행문자나 EOF가 들어오지 않고 버퍼가 꽉 찼을 때 어떻게 되는가?
=s[0:MAX-1]= 이 모두 유효한 문자들이지만, 널 문자로 끝나야 한다는 문자열
제약 탓에 마지막 문자가 널 문자로 대체되고 무시될 것이다.

경계조건 테스트를 습관화하면 이런 구멍들을 빨리 메울 수 있다.

*** 사전, 사후 조건을 테스트하라

입력 값의 범위 검사하는 것은 사전조건 테스트의 흔한 예시이다. 사전
조건을 검증하지 않고 무시하는 것은 시스템에 어떤 문제를 불러일으킬지
모르기 때문에 유의해야 한다.

*** 단정문을 사용하라

C와 C++ 언어는 =<assert.h>= 헤더파일을 통해 사전 조건과 사후 조건
테스트를 넣을 수 있는 단정문 기능을 제공한다. 어디서 문제가
발생했는지에 관해 이 단정문들은 유용한 정보를 제공할 것이다.

*** 방어적으로 프로그래밍하라

'말도 안되는' 경우를 처리하라. 정말 있을 수 없는 경우지만 어디선가
발생한 문제에 의해 그런 말도안되는 일이 벌어질 수도 있다. 그러므로
일단은 검증하는 것이다. 프로그램을 오용하거나 잘못된 데이터가 들어온
경우에도 버틸 수 있도록 프로그래밍해야 한다.

이런 방어적 프로그래밍은 널 포인터, 범위를 벗어난 인덱스, 0으로 나누기
등의 에러를 잡아내는 데 흔히 쓰인다.

*** 리턴값을 검사하라

'당연히 동작할' 시스템 호출 함수들을 쓸 때도 리턴값 검사를 게을리 하지
않으면 정말 예외적인 상황에도 프로그램 동작을 유지시키고, 에러를
수복하는 데 도움이 될 것이다. 대표적으로 파일 입출력 함수들이 그러하다.

*** 왜 하는가

개발하면서 하는 테스트는 조금은 번거롭지만 후한 결과를 가져다
준다. 프로그램을 작성하면서 동시에 테스트를 생각하면 장애
허용(fault-tolerant) 시스템을 만드는데 도움이 되는 코드를 쓸 수 있다.

이런 조건들을 당연하다 여기고 검사하기를 경시하면, 그런 조건을 모두
잊어버릴 때 쯤에 프로그램에 잠재된 버그가 튀어나와서 프로그래머를
괴롭힐 것이다.
...
다음 예제들을 경계조건에서 테스트하고, 1장에서 나온 스타일 원칙과
조언에 맞게 필요한 만큼 수정하라.
...
트릭같은 코드 =while(n--)= 는 곧바로 불길한 예감이 들게 한다. 숫자만큼
반복함에도 for-idiom을 쓰지 않아 코드 의도를 이해하기 어렵다. 심지어
이 코드는 일반적인 경우에조차 정상적으로 동작하지 않는다. fac에 n을
곱하기도 전에 1을 빼버린다. 경계 조건(n < 0)에서 무한루프에 빠지는
것은 말할것도 없다.
...
어떤 문자열에 들어있는 문자들을 한 줄에 하나씩 출력한다.
...
역시 문자열을 일렬로 순회함에도 불구하고 for-idiom을 사용하지
않았다. do-while로 쓰여있어 경계조건(빈 문자열)에서 제대로 동작하지도
않는다.
...
원본(src)에서 대상(dest)으로 문자열을 복사한다.

#+BEGIN_SRC c
  void strpy(char *dest, char *src)
  {
      int i;
...
인덱스가 =src= 의 끝에 도달하면 루프가 종료되기 때문에 =dest= 에
널문자가 삽입되지 않는다.
...
역시 문자열을 복사한다. n개의 문자를 s에서 t로 복사한다.
...
케이스 (c)와 마찬가지로 마지막 인덱스에 도달했을 때 문제가
생긴다. 또한 순차 접근이므로 for-idiom을 사용하도록 고칠 것이다.
...
여기서 경계조건이라고 할만한 것은 없는 것 같다. 그러나 알파벳의 first
half 경계가 L이라는 구문은 틀렸다. 알파벳의 열 세번째 문자는 N이다.
...
우리가 1998년 말에 이 책을 쓰고 있을때는 Y2K 문제가 사상 최대의 경계
조건 문제로 떠올랐다.

*문제 (a)* 시스템이 2000년에도 작동할 수 있을지 검사하기 위해 어떤
날짜를 이용하겠는가? 이 테스트 수행에 비용이 많이 든다고 할 때, 2000년
1월 1일을 시도해 본 다음 어떤 순서로 테스트해보겠는가?

*나의 생각*

예를 들어 기준 시점을 시작으로 하는 주식 프로그램을 생각해보자. 이
프로그램이 1957년을 시작으로 해서 현재까지의 주식 시장 거래 내용과
경제 상황을 바탕으로 지수를 예측한다고 가정해보자. 이런 프로그램이라면
테스트하는 비용도 아주 비쌀 것이다. 

이 프로그램의 Y2K버그를 테스트하는 용도로 2000년이라는 데이터는
충분하지 않다. 2000년이 1900년으로 오버플로우 되더라도 1900년대의
데이터가 없기 때문에 데이터가 덮어써지지 않아 별 상관이 없을
것이다. 입력 값으로 2099년도를 넣어본다면, 아마 2056년(으로 의도된
1956년)에서 다음 해로 넘어갈 때 1957년으로 인식되어 엉뚱한 예측
데이터가 나오는 문제가 생길 것이다.

이 프로그램이 만약 연도의 세자리 수까지 저장하여 1957년을 원년으로
지나간 햇수를 저장한다 쳐도, 2956년에서 2957년으로 넘어갈 때 문제가
생길 테니 그 시점을 테스트해보면 된다.

*문제 (b)* 다음처럼 날짜를 문자열 형태로 리턴하는 표준 함수인 =ctime=
함수를 어떻게 테스트하겠는가?
...
자신의 프로그램이 ctime을 호출한다고 가정할 때, 결함있는 코드에
대비하기 위해 어떤 식으로 방어적 프로그래밍을 하겠는가?

*나의 생각*

연도가 2000, 2099, 2999년 같은 값인 경우의 경계 조건을 테스트한다.
만약 =ctime= 함수에 문제가 있다면, =ctime= 으로 받아온 문자열을
입력값에 맞게 직접 수정해야 할것이다.

*문제 (c)* 다음과 같은 형식으로 결과를 출력하는 달력 프로그램을
테스트할 방법을 논하라.
...
이 역시 연도를 바꾸어가며 경계 조건을 테스트하면 되겠다.

*문제 (d)* 자신이 쓰고 있는 시스템에서 시간 경계 조건으로 그 밖에 어떤
것이 있겠는가? 그런 조건을 제대로 처리할 수 있는지 알기 위해 어떤
식으로 테스트할 것인가?

*나의 생각*

기준시점으로부터 몇 초가 지났는지 알려주는 =mktime= 과 같은 함수는
적절한 =time_t= 를 반환하는데, 이런 함수들은 주어진 시점이 기준
시점보다 더 이전 시점인 경우가 경계 조건이 될 것이다.

** 체계적인 테스트

주도면밀하게 테스트를 해서 빠뜨리고 지나가는 부분이 없어야 하고,
결과를 계속 기록해서 얼마나 확인을 끝낸건지 알 수 있어야 한다.

*** 점층적으로 테스트하라

테스트는 프로그램을 구축하면서 차근 차근 해나가야 한다. '빅 뱅'
테스트는 점층적인 접근 방식보다 훨씬 어렵고 시간도 더 잡아먹는다.
프로그램 일부를 작성하고, 테스트하고, 몇몇 코드를 더 추가하고,
테스트하는 방식이어야 한다.

- '빅 뱅' 테스트 :: 프로그램을 전부 작성한 다음 한꺼번에 테스트 하는
               방식을 얘기한다.

만약 두 패키지를 각각 따로 작성하고 테스트했다면, 마지막에 둘을 연결할
때 같이 잘 돌아가는지도 테스트한다.

*** 단순한 부분을 먼저 테스트하라

점층적 접근 방식은 기능테스트를 할 때도 적용된다. 가장 단순하면서도
가장 일반적으로 실행되는 기능 부분에 먼저 집중해야 한다. 이 부분이
제대로 돌아가게 된 다음에 더 많은 부분을 테스트한다. 각 테스트는 다음
테스트를 돕는 최소한의 역할을 수행한다.

이 절에서는 효과적인 테스트를 선택하는 방법과 그 테스트들을 어떤
순서로 적용할 것인가에 대해 논한다.

*정수 배열 이진 탐색 함수의 예시*

복잡성이 증가하는 순서로 테스트를 수행한다.

#+BEGIN_EXAMPLE
  - 아무 원소도 없는 배열을 탐색한다.
  - 한 개의 원소가 있는 배열에서 다음과 같은 값을 탐색한다.
    + 배열에 있는 원소보다 작은 값
    + 배열에 있는 원소와 동일한 값
    + 배열에 있는 원소보다 큰 값
  - 두 개의 원소가 있는 배열에서 다음과 같은 값을 탐색한다.
    + 다섯가지의 가능한 경우를 모두 검사한다.
  - 이 배열에 있는 두 원소가 동일할 때 다음과 같은 경우에 생길 일을 검사한다.
    + 배열에 있는 원소보다 작은 값
    + 배열에 있는 원소와 동일한 값
    + 배열에 있는 원소보다 큰 값
  - 세 개의 원소가 있는 배열에서 검사한다.
  - 네 개의 원소가 있는 배열에서 검사한다.
#+END_EXAMPLE

이 함수가 문제없이 이런 난관들을 통과한다면 아마 괜찮다고 봐도
되겠지만, 더 철저히 테스트할 수도 있고, 테스트가 더 많아지면 이런
프로세스를 시스템화 하기 위해 테스트를 작업발판(scaffold)으로 구축하는
편이 더 나을 것이다.

다음의 구동 프로그램은 상당히 단순하므로 다루기도 쉽다. 탐색할 키가
있는 입력 줄과 배열 크기를 읽어서, 그 크기만한 배열을 생성하고 1, 3,
5, ... 식의 값을 넣는다. 그리고 이 배열에서 키를 탐색한다.

#+BEGIN_SRC c
  /* bintest main: binsearch를 테스트하는 작업발판 */
  int main(void)
  {
...
단순하지만 유용한 작업발판이고, 얼마든지 작은 수정을 통해 다양한
테스트를 수행할 수 있다.

*** 어떤 결과가 나와야 하는지 알라

테스트를 수행하기 위해 정답을 알고 있어야 하는 것은 어찌 보면
당연하다. 그러나 종종 정답이라고 확인할만한 기준을 찾기가 어려운
경우도 있다.

컴파일러를 만들 때, 수학적 알고리즘을 구현할 때, 그래픽 프로그램을
검증하는 것 등이 그렇다.

- 컴파일러라면 테스트용 코드를 컴파일하고 실행했을 때, 그 실행 결과가
  올바른 결과를 출력하는지 보는 수밖에 없다.
- 수학적 프로그램을 테스트할 때는 단순한 경우부터 어려운 경우까지
  알고리즘의 양 극단 사이를 탐색할 수 있는 테스트 케이스를 만들어야
  한다.
- 그래픽 프로그램을 테스트할 때는 직사각형 하나를 그렸는지 볼 뿐만
  아니라, 꼭지점이 정위치에 있는지도 확인해보아야 한다.

만약 프로그램 수행 결과를 입력값으로 되돌릴 수 있는 기능이 있다면 그것
또한 함께 테스트하라. 암호화와 복호화는 역의 관계이다. 암호화는
가능한데 복호화는 불가하다면 뭔가 잘못된 것이다.

*** 보존 속성을 검증하라

많은 프로그램은 입력의 특정 속성을 보존한다. 이런 보존 속성은 프로그램
내에서 검증하는것이 가장 간편하다. 데이터 구조에 든 원소 개수를 세는
함수로 단순한 일관성 검사를 할 수도 있다. 이런 조건들은 검증하기
어렵지 않다. 그러나 프로그램 내에서 보존 속성을 검증하기 어려운
경우들도 있다. 이런 경우에는 외부 도구의 도움을 받는 방법도 있다.

전통적인 유닉스 툴킷들이 보존 속성을 테스트하는데 사용될 수 있다. wc,
sum, cmp, diff 같은 프로그램을 통해 내 소프트웨어가 보존 속성을 잘
관리하고 있는지 파악할 수 있다.

바이트 빈도 보존을 검증할 수 있는 =freq= 프로그램을 만들어본다.
...
라이브러리나 프로그램의 독립적인 구현 버전을 비교하면 같은 답이 나와야
한다. 예를 들어, 두 컴파일러는 같은 컴퓨터에서 같은 방식으로 동작하는
프로그램을 생성해야 한다.

두 가지 다른 방식으로 답을 계산할 수 있거나, 좀 느리지만 확실한 답을
내놓는 프로그램을 만들어서 비교 검증하는 방법도 있다. 두 프로그램이
같은 답을 내놓는다면 답이 맞을 가능성이 높다는 뜻이다.

*** 테스트 범위를 측정하라

테스트들이 프로그램의 모든 코드를 빠짐없이 테스트하는 것을 목표로
삼으라. '일어날 수 없는' 케이스는 제외하더라도, 평범한 입력을 가지고
모든 케이스에 도달하게 만드는 것은 상당히 어렵다.

이를 위해 프로파일러 등을 이용한 도움을 받을 수 있다. 프로파일러는
특정 코드 부분이 얼마나 실행되었는지 빈도수를 체크해주기도 한다.

** 연습

*** 연습 6-3.

freq를 어떻게 테스트할 것인지 설명하라.

*해설*

먼저 빈 입력에 대해서 테스트 한다. 그 다음은 문자 하나만을 가진 입력을
테스트한다. 이 때 (비출력 문자를 포함해) 모든 아스키 문자를 테스트할
수 있도록 한다. 그 다음은 두개, 세개 등으로 범위를 늘려가며 테스트
한다. 테스트 입력을 쉽게 만들 수 있게 scaffold 프로그램을 만들 수도 있다.
...
이 프로그램의 예시 입력과 출력은 다음과 같다.

#+BEGIN_EXAMPLE
  예시 입력: 33 3 97 3 48 3
  예시 출력: !!!aaa000
#+END_EXAMPLE

다른 방식의 scaffold 프로그램을 만들 수도 있다.
...
#+BEGIN_EXAMPLE
  예시 입력: 65 90 89 65
  예시 출력: ABCDEFGHIJKLMNOPQRSTUVWXYZYXWVUTSRQPONMLKJIHGFEDCBA
#+END_EXAMPLE

*** 연습 6-4.

다른 데이터 값, 예를 들어 32비트 정수나 부동소수 값이 발생하는
빈도수를 측정할 수 있는 freq를 설계하고 구현하라. 세련된 방식으로
다양한 타입 값을 모두 처리할 수 있는 프로그램을 만들 수 있겠는가?

*목표*

C언어가 처리할 수 있는 타입 값은 기본적으로 정수, 부동소수점 실수,
문자값이다. 목표를 좀 더 쉽게 달성할 수 있도록, 입력 값에 부호있는
정수, 혹은 실수가 포함되지 않는다고 가정하고, 정수나 실수 값의
오버플로우 가능성을 고려하지 않는다고 하자.

*알고리즘*

수 판별 알고리즘을 생각해보자. 버퍼에 담은 문자들을 토대로 수를
판별해야 하는 이상, 이전에 결정된 타입에 영향을 받게 된다. 예를 들어,
버퍼로 처음 읽은 입력이 'xxx123.23'이고 그 다음 입력이 '456yyy'이라면,
실제 입력은 'xxx123.23456yyy'로 부동소수점 하나로 처리해야 한다.
...
#+BEGIN_EXAMPLE
  ...
  이전 타입에 대해
  (전제: 이전 타입이 fltnum, fixnum 이면서 '.'로 끝나는 경우는 없다)
  이전 타입이 문자일 때
  - 이전 타입은 상관없다
  이전 타입이 fixnum일 때
  - '.'으로 시작하면
    - 수열이 뒤따르면:fixnum--, fltnum++
    - 그렇지 않으면:chr('.')
  - 수열로 시작하면
    - '.'으로 끝나면
      - 또 수열이 뒤따르면:fixnum--, fltnum++
      - 그렇지 않으면:skip
    - 그렇지 않으면:skip
  - 문자로 시작하면:chr
  이전 타입이 fltnum일때, 혹은 이전에 fixnum->fltnum 변경이 일어났을 때
  - 수열로 시작하면:skip
  - 그렇지 않으면: 이전 타입은 상관없다
#+END_EXAMPLE
...
타입이 세개뿐임에도 불구하고, 타입을 판별하는 조건이 까다로워 다시는
보고싶지 않은 로직이 완성 되었다. 프로그램이 정상 동작하는지 확인하기
위해, =BUFSIZ= 매크로 값을 10으로 재조정하고 여러 입력을 테스트해본다.
...
#+BEGIN_EXAMPLE
  <입력:
  123433333333333333333.22222222222222221111.12345.x.x.x
  (줄바꿈)
  >출력:
  fixnum : 0
  float : 2
...
기쁘게도, 정상 작동한다. 로직을 수정할 필요는 없어 보인다.

*세련된 방식으로 다양한 타입 값을 모두 처리할 수 있는 프로그램을 만들 수 있겠는가?*

오~ 사절.

** 테스트 자동화

손으로 일일이 테스트하기보다는, 자동화된 테스트 스크립트를 통해 보다
자주, 그리고 더 많은 테스트 케이스로 테스트를 하는 것이 바람직하다.

*** 회귀 테스트를 자동화하라

- 회귀 테스트 :: 새 버전과 예전 버전과 비교하는 모든 테스트를 수행한다.

문제를 고쳤을 때, 다른 부분이 또 다시 망가지지 않았나 확인해야 하고,
다른 부분과의 이음새가 잘 맞는지도 테스트 해보아야 한다. 회귀 테스트의
목적은 대상의 동작 방식이 예상 외의 방식으로 변하지 않았다는 것을
확인하는 것이다.

이런 테스트를 자동화하기 위해, 시스템 유틸리티의 도움을 받으면 좋다.
다음은 ka 라는 가상의 애플리케이션의 구버전(old-ka)과 새
버전(new-ka)을 테스트하는 쉘 스크립트이다.

#+BEGIN_SRC sh
  for i in ka_data.*              # 테스트 데이터에 대해 루프를 돈다
  do
      old_ka $i > out1            # 이전 버전을 테스트
      new_ka $i > out2            # 새 버전을 테스트
      if ! cmp -s out1 out2       # 두 결과를 비교한다
      then
          echo $i: BAD            # 차이가 나면 에러 메시지를 출력한다
      fi
  done
#+END_SRC

테스트 스크립트는 예상치 못한 일이 일어날 때만 표시를 남겨야 한다.
잘못되지 않았을 때도 뭔가 출력하려고 하면 에러와 구분하기 어렵기
때문이다. 물론 진척도를 보여주는 것은 좋으나, 그럴 경우 에러와 확연히
구별되도록 해야 할 것이다.

*참고* =cmp= 명령에 s 옵션을 주면 상태만 보고하고 연산 결과는 출력하지
않는다.

회귀 테스트에서는 이전 버전이 정확한 답을 낸다는 것을 기본 가정으로
삼는다. 이 가정은 처음부터 주의 깊게 확인해야 하고 계속 철저히 지켜야
한다. 만약 문제 있는 답이 슬그머니 끼어들어오면, 적발도 쉽지 않을
뿐더라 그 이후로 그 답에 의존하는 모든 것들이 틀리게 된다. 회귀 테스트
자체가 정상인지 주기적으로 점검하는 습관을 들이자.

*** 자급자족형 테스트를 창조하라

회귀 테스트는 입력에 대한 두 버전의 프로그램의 출력을 비교하는데 반해,
자급자족형 테스트는 입력에 대한 답을 미리 적어두고 프로그램의 출력과
비교한다. 두 테스트 방식은 상호 보완적 관계에 있어, 회귀 테스트가
어려운 경우(비교 대상이 없는 경우)에는 자급자족형 테스트를 이용하는
것이 적합하다.

저자의 awk 프로그램 테스트용 DSL이 인상적이라서 가져와보았다.
...
첫 줄의 try 블록의 내용이 awk가 실행할 프로그램이고, 뒤따라 두 열로
구분된 값들은 각각 입력과 기대 출력값이다. 저자는 이런 방식으로 수
많은 awk 테스트 프로그램을 만들었고, 종종 유용한 도움을 주었다고 한다.

- 버그를 발견했다면, 그 문제에 해당하는 테스트를 새로 추가해야 한다.
- 절대 테스트를 없애지 말라. 버그를 없앴는지 확인할 뿐만 아니라, 회귀
  테스트에도 유용하다.
...
가능한 한 컴퓨터의 도움을 활용해서 printf를 위한 테스트 집합을 설계하라.

*어떻게 할 것인가*

컴파일러가 한 개뿐인 경우, 두 프로그램의 출력을 비교 검정하는 것은
쉽지 않다. 따라서 답을 모두 아는 상태에서 하는 수 밖에 없다. 테스트를
더 용이하게 하기 위해, 자급자족 테스트를 위한 프레임워크를 만들것이다.

저자의 DSL이 인상깊어, 이와 비슷하게 해보고자 한다. 대신 문법을 좀 더
단순화 한다. 테스트 DSL의 형식은 다음과 같다.
...
test condition에는 어떤 조건식이라도 들어갈 수 있다. 예를 들면 다음과 같다.

#+BEGIN_EXAMPLE
  "", 1, 2, 3
  [ 0 -eq `stat -c%s out.txt`]
#+END_EXAMPLE

*DSL 실행기 구현*

프로그램 작성과 실행은 이렇게 하면 된다.
...
홀수 행에는 테스트가, 짝수 행에는 조건식이 있으므로 테스트 프로그램은
적당히 이런 골격을 가지게 된다.
...
주석과 빈 라인을 실행할 수 있도록 만든 완전한 DSL 실행기는 다음과 같다.
...
반드시 실패하는 테스트 말고 전부 올바르게 설계되었다.

*출력을 만들어내는 프로그램*

테스트를 일일이 만들어내는 것도 귀찮게 되었다. 좀 더 현명하게
게을러지려면, 테스트의 답을 자동으로 만들어낼 수 있어야 한다. 빈
입력을 검증하는 테스트를 빼면 모두 파일 내용을 검증하기 때문에, 이같은
조건식을 자동 생성해주는 쉘 스크립트를 작성한다. 이 작업은 현재 버전의
c 컴파일러가 올바르게 작동한다는 가정 하에 만드는 것이다.
...
=cc= 와 같은 프로그램이 오류를 내보내면 어떻게 해야할지는 잘 모르겠다.
테스트 DSL이 올바르게 작성되었다고 믿는 수밖에 없다.

*출력을 만들어내는 프로그램* 의 입력조차 자동으로 만들어낼 수 있다면
좋겠지만, 거기까진 어떻게 해야할 지 잘 모르겠다.
