...
저자의 DSL이 인상깊어, 이와 비슷하게 해보고자 한다. 좀 더 단순화
한다. 테스트 DSL의 형식은 다음과 같다.
...
test condition에는 어떤 명령이라도 들어갈 수 있다. 예를 들면 다음과 같다.

#+BEGIN_EXAMPLE
  "", 1, 2, 3
  expr 0 = `stat -c"%s" out.txt`
#+END_EXAMPLE
